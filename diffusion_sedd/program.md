...
3. **Read the in-scope files**: The repo is small. Read these files for full context:
   ...
   - `train.py` / `model.py` — the files you modify. Model architecture, optimizer, training loop.
...
Each experiment runs on a single GPU. The training script runs for a **fixed time budget of 5 minutes** (wall clock training time, excluding startup/compilation). You launch it from inside the experiment folder, e.g.: `cd diffusion_sedd && uv run train.py`.

**What you CAN do:**
- Modify the experiment's training script (`train.py` / `model.py`) — this is the only file you edit. Everything is fair game: model architecture, optimizer, hyperparameters, training loop, batch size, model size, etc.
...
The script prints an eval line every `eval_interval` steps, like this:

```
Step 9000/10000 : train 0.5421 | val 0.5503 | lr 3.00e-04 | eval 1.20s | gpu 12.34GB
```

The metric we optimize is the **validation loss** (`val`) — the diffusion-weighted denoising score entropy on the held-out split. Lower is better.

Note that the script is configured to always stop after 5 minutes, so depending on the computing platform of this computer the numbers might look different. You can extract the final (best) val loss and peak memory from the log file:
...
3. peak memory in GB, round to .1f (e.g. 12.3 — read the `gpu` field from the last eval line, already in GB) — use 0.0 for crashes
...
2. Tune the experiment's training script (`train.py` / `model.py`) with an experimental idea by directly hacking the code.
3. git commit
4. Run the experiment from inside its folder: `conda activate; python train.py > run.log 2>&1` (redirect everything — do NOT use tee or let output flood your context)
