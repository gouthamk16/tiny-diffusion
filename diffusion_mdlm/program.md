...
Each experiment runs on a single GPU. The training script runs for a **fixed time budget of 5 minutes** (wall clock training time, excluding startup/compilation). You launch it from inside the experiment folder, e.g.: `cd diffusion_mdlm && uv run train.py`.
